...
export default function ProgressoBanner({ posts: initialPosts }: { posts: Post[] }) {
  ...
  const done    = posts.filter(postCompleto).length
  const allDone = total > 0 && done === total
  const progress = total > 0 ? Math.round((done / total) * 100) : 0

  // Detecta se há posts em fase 'arte' para customizar o texto
  const temArte = posts.some((p) => p.fase === 'arte')
    ...
    <>
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-4 mb-5">
        <div className="flex items-center justify-between text-sm mb-2">
          ...
          <span className="text-gray-500">{done} de {total} {temArte ? 'posts completos' : 'copys avaliadas'}</span>
        </div>
